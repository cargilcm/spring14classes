...
char *my_name = "Ted";
char my_name2[] = "Ted";
/** most likely (since all strings are terminated in ANSI C compiled programs with null at their end
 * that each param in argv is terminated as such.
 */
int main(int argc,char *argv[]) {

	int *argPre = my_name;
	int *argPre2 = my_name2;

	char *argOne = argv[1];
	char *argTwo = argv[2];

	printf("argv[1],[2] BEFORE: %s, %s\n",argv[1],argv[2]);
	cgi_decode(argv[1],argv[2]);
	printf("argv[1],[2] AFTER: %s, %s\n",argv[1],argv[2]);
...
//		printf("&c: %s",&c);
		if(c=='+'){
			*dptr=' ';
			...
			int digit_low = 16;// the above all makes sense with pointer arithmetic and explanations since 1 byte can range 0-16
			//Hex_Values[*(++eptr)];
